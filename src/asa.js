...
class meters {
    ...
    init() {
        parent.$('#bottomleftcorner2').prepend(
    ...
    tick() {
        ...
        parent.$('#meters').find('#goldmeter').html('Gold: ' + this.gold.toLocaleString() + ' (' + (Math.round(this.gold / passedh).toLocaleString() + ')' || 0));
        parent.$('#meters').find('#xpmeter').html('XP: ' + gained_xp.toLocaleString() + ' (' + (Math.round(gained_xp / passedh).toLocaleString() + ') TL: ' + ((character.max_xp - character.xp) / Math.round(gained_xp / passedh)).toFixed(2)) || 0);
    }
    dps_table() {
    ...
    on_destroy() {
        parent.$('#bottomleftcorner2').find('#meters').remove();
