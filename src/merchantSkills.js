...
function sellTrash() {
    for (let i = 0; i <= 34; i++) {
        if (character.items[i] && trashName.indexOf(character.items[i].name) !== -1 && !item_grade(character.items[i]) > 0) {
...
function exchangeGems() {
    for (let i = 0; i <= 34; i++) {
        if (character.items[i] && (G.items[character.items[i].name].type === "gem" || G.items[character.items[i].name].type === "box")) {
            exchange(i);
            log("Item Exchanged!");
        }
    }
...
function depositItems() {
    for (let i = 0; i <= 34; i++) {
        if (character.items[i] && (character.items[i].level && character.items[i].level > sellItemLevel) || item_grade(character.items[i]) > 1) {
            bank_store(i);
...
function merchantsLuck() {
    for (i in parent.entities) {
        if (parent.entities[i].player
            && parent.entities[i].ctype !== "merchant"  // Verify they are not a merchant
            && !parent.entities[i].npc // Not and NPC
            && !parent.entities[i].s.mluck // Does not have Buff
            && character.mp > (character.max_mp * MPReserve) // We have mana above reserve
            && character.mp > G.skills.mluck.mp // We have more mana than spell costs
            && distance(character, parent.entities[i]) < G.skills.mluck.range // range to entitie is within range
            && can_use("mluck")) { // can use mluck
            if (!parent.entities[i].s.mluck || parent.entities[i].s.mluck.f !== "AliVezas") {
                use_skill("mluck", parent.entities[i].name);  // cast mluck
                log("Buffing: " + parent.entities[i].name); // tell us we used it
            }
        }
    }
}
