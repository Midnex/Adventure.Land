function loadCharacters() {
    start_character("Casyl", "MainLoop");
    start_character("RangeRick", "MainLoop");
    start_character("Duckabush", "MainLoop");
    log("Loading Characters...");
    setTimeout(initParty, 8000);
}

function initParty() {
    send_party_invite("Casyl");
    send_party_invite("RangeRick");
    send_party_invite("Duckabush");
    log("Party Invites sent!");
}

function stopCharacters() {
    stop_character("Casyl");
    stop_character("RangeRick");
    stop_character("Duckabush");
...
function transferLoot(merchantName) {
    ...
    if (character.ctype !== "merchant"
        ...
        // Transfer Gold
        if (character.gold > 500000) send_gold(merchant, character.gold * 0.9)
        // Transfer Items
...
function relocateItems() {
    ...
    if (locate_item("hpot1") !== -1 && locate_item("hpot1") !== 35) swap(locate_item("hpot1"), 35);
    if (locate_item("mpot1") !== -1 && locate_item("mpot1") !== 36) swap(locate_item("mpot1"), 36);
    if (locate_item("hpot0") !== -1 && locate_item("hpot0") !== 37) swap(locate_item("hpot0"), 37);
    if (locate_item("mpot0") !== -1 && locate_item("mpot0") !== 38) swap(locate_item("mpot0"), 38);
    //Compound Scroll
    if (locate_item("cscroll0") !== -1 && locate_item("cscroll0") !== 39) swap(locate_item("cscroll0"), 39);
    //Upgrade Scroll
    if (locate_item("scroll0") !== -1 && locate_item("scroll0") !== 40) swap(locate_item("scroll0"), 40);
}
...
function usePotions(healthPotThreshold = 0.9, manaPotThreshold = 0.9) {
    if (!character.rip && (character.hp < (character.max_hp - 400) || character.mp < (character.max_mp - 500))) use_hp_or_mp();
}