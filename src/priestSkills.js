load_code(96) // Filter Log
load_code(97) // Party Invite
load_code(98) // Use Potion
load_code(99) // DPS Meter

map_key("Q", "snippet", "merchant()")

function merchant() {
	for (i = 0; i < 35; i++) { send_item("AliVezas", i, 9999);}
}

game_log("---Script Start---");
// Put monsters you want to kill in here
var monster_targets = ["phoenix", "frog", "tortoise", "snowman", "greenjr"];

var state = "farm";

var min_potions = 1000; // The number of potions at which to do a resupply run.
var purchase_amount = 4500; // How many potions to buy at once.
...
setInterval(function () { use_skill("darkblessing") }, 60000);
		...
		if (target != null) {
			if (distance_to_point(target.real_x, target.real_y) < character.range) {
				if (can_attack(target)) {
					if (target["max_hp"] > 3000) { use_skill("curse", target) };
					attack(target);
				}
			}
			else {
...
function resupply_potions() {
	var map = parent.character.map
	if (map == "winterland" || map == "winter_inn") {
		var potion_merchant = get_npc("Warlin");
	}
	else {
...
function lowest_health_partymember() {
	...
	if (parent.party_list.length > 0) {
		for (id in parent.party_list) {
			...
			if (entity != null) party.push({ name: member, entity: entity });
		}
	}
	else {
